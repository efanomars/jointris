...
def testAll(sBuildType, sInstallDir, sSudo, bBuildStatic, bSanitize):
	...
	subprocess.check_call("./scripts/uninstall_jointris-all.py -y --installdir {}  {}".format(sInstallDir, sSudo).split())

	subprocess.check_call("./scripts/install_jointris-all.py -b {}  -t On  -d On --docs-to-log  --installdir {}  {} {} {}"\
			.format(sBuildType, sInstallDir, sSudo, sStatic, sSanitize).split())


	os.chdir("libstmm-jointris/build")
	subprocess.check_call(sSanitizeOptions + " make test", shell=True)
	if os.path.getsize("libstmm-jointris_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-jointris_doxy.log not empty")
	os.chdir("../..")

	os.chdir("libstmm-jointris-xml/build")
	subprocess.check_call("make test", shell=True)
	if os.path.getsize("libstmm-jointris-xml_doxy.log") > 0:
		raise RuntimeError("Error: libstmm-jointris-xml_doxy.log not empty")
	os.chdir("../..")

	os.chdir("jointris/build")
	subprocess.check_call("make test", shell=True)
	os.chdir("../..")
...
def main():
	...
	print("------------------------------")
	print("testall.py (jointris) started!")
	print("------------------------------")
	...
	print("--------------------------------------------")
	print("testall.py (jointris) finished successfully!")
	print("--------------------------------------------")
