...
# File:   uninstall_libstmm-jointris-xml.py

# Removes all files installed by the install_libstmm-jointris-xml.py script.
...
def main():
	import argparse
	oParser = argparse.ArgumentParser("Removes all files created by install_libstmm-jointris-xml.py")
	...
	if not oArgs.bNoUninstall:
		subprocess.check_call("{} rm -r -f            {}/include/stmm-jointris-xml".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f             {}/lib/libstmm-jointris-xml.so*".format(sSudo, sInstallDir), shell=True)
		subprocess.check_call("{} rm    -f             {}/lib/libstmm-jointris-xml.a".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm    -f      {}/lib/pkgconfig/stmm-jointris-xml.pc".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f       {}/share/doc/libstmm-jointris-xml".format(sSudo, sInstallDir).split())
		subprocess.check_call("{} rm -r -f {}/share/stmm-games/themes/jointris/simple_j.thm".format(sSudo, sInstallDir).split())

	if not oArgs.bNoClean:
		os.chdir("..")
		subprocess.check_call("{} rm -r -f libstmm-jointris-xml/build".format(sSudo).split())
